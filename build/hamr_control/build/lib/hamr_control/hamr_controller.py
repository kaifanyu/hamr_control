...
class HamrControlNode(Node):
    def __init__(self):
        ...
        PID_default_gains = {
            "P_x": 1.0, "I_x": 0.4, "D_x": 0.25,
            "P_y": 1.0, "I_y": 0.4, "D_y": 0.25,
            "P_yaw": 8.0, "I_yaw": 0.4, "D_yaw": 0.1,
        }
        ...
        self.use_diff_drive = False  # False: full holonomic (base + turret), tracks base x-y in any direction
    ...
    def pid_step(self):
        ...
        def compute_errors():
            ...
            # Measure the error at the point the Jacobian actually controls:
            # the turret axis, offset b along the base heading from the odom
            # origin. Measuring at the odom origin instead couples base
            # rotation into the x-y error on the |b| lever arm, which makes
            # the robot orbit the reference (~2|b| diameter circles) whenever
            # terrain keeps disturbing the base heading.
            b = self.hamr_config["b_wheel"]
            ctrl_x = self.pose_base_.pose.position.x + b * math.cos(yaw_base_w)
            ctrl_y = self.pose_base_.pose.position.y + b * math.sin(yaw_base_w)
            err_x = self.reference_.x - ctrl_x
            err_y = self.reference_.y - ctrl_y
    ...
    def compute_velocities(self, desired_velocity, yaw):
        ...
        if self.use_diff_drive:
            ...
            # standard diff-drive: base drives along its heading and rotates;
            # turret is not used (matches current hardware behavior)
