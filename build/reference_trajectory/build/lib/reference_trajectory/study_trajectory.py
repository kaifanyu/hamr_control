...
class StudyTrajectoryNode(Node):
    """Publish a selected profile translated to one captured Vicon XY pose."""
    ...
    def _capture_start(self, now):
        ...
        origin_x, origin_y, start_yaw, frame_id = self.latest_finite_odom
        self.captured_origin = (origin_x, origin_y, start_yaw, frame_id)
        self.trajectory = TranslatedTrajectory(
            self.canonical_trajectory, origin_x, origin_y
        )
        ...
        self.get_logger().info(
            ...
            % (origin_x, origin_y, frame_id, self.startup_hold_s)
        )
        return True

    def _publish_metadata(self):
        origin_x, origin_y, start_yaw, frame_id = self.captured_origin
